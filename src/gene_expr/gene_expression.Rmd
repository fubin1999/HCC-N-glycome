...
write_csv(volcano_data, "results/source_data/Figure_6b.csv")
```
...
color_fun <- colorRamp2(c(-2, 0, 2), c("blue", "black", "red"))
...
write_csv(as.data.frame(scaled_expr_mat), "results/source_data/Figure_6a.csv")
...
write_csv(plot_data, "results/source_data/Figure_6c.csv")
```
...
write_csv(plot_data, "results/source_data/Supplementary_Figure_20h.csv")
```
...
walk2(
  letters[1:7],
  geo_volcano_data,
  ~ write_csv(.y, str_glue("results/source_data/Supplementary_Figure_20{.x}.csv"))
)
```
...
```{r KM-plot}
survival_results <- read_csv("results/data/TCGA/survival_results.csv")
...
draw_KM_for_gene <- function(gene, event) {
  ...
  ggsurvplot(
    ...
    labs(
      ...
      subtitle = str_glue("HR={round(hr, 2)}, p={scales::scientific(p_value)}"),
      ) +
...
survival_results %>%
  filter(gene == "B4GALT2", event == "OS") %>%
  write_csv("results/source_data/Figure_6d.csv")

survival_results %>%
  filter(gene == "B4GALT2", event == "DSS") %>%
  write_csv("results/source_data/Figure_6e.csv")

survival_results %>%
  filter(gene == "B4GALT3", event == "PFI") %>%
  write_csv("results/source_data/Figure_6f.csv")

survival_results %>%
  filter(gene == "B4GALT3", event == "DFI") %>%
  write_csv("results/source_data/Figure_6g.csv")
```
