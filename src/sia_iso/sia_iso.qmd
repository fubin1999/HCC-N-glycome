...
glycan_properties <- read_csv("data/human_serum_glycans.csv") |>
  mutate(structure = parse_glycoct(structure)) |>
  pull(structure) |>
  get_meta_properties() |>
  mutate(glycan = read_csv("data/human_serum_glycans.csv")$composition)
...
sia_derived_traits <- abundance |> 
  ...
  select(-nS) |> 
  left_join(
    glycan_properties |> select(glycan, n_gal = nG), 
...
write_csv(post_hoc_result, "results/source_data/Supplementary_Figure_9.csv")
```