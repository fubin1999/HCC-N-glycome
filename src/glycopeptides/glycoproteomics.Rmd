...
n_glycoforms <- n_distinct(raw_data$protein, raw_data$protein_site, raw_data$glycan_composition)
n_glyosites <- n_distinct(raw_data$protein, raw_data$protein_site)
n_glycoproteins <- n_distinct(raw_data$protein)
...
```{r plot_summary}
n_summary_plot <- tribble(
  ~n_type, ~n,
  "Glycopeptides", n_glycopeptides,
  "Glycoforms", n_glycoforms,
  "Glycosites", n_glyosites,
  "Glycoproteins", n_glycoproteins,
  "Glycans", n_glycans,
) |> 
  ggplot(aes(n, reorder(n_type, n))) +
  geom_col(fill = "#D6C89B", color = "#88642d") +
  geom_text(aes(label = n), nudge_x = 120) +
...
ggsave("results/figures/glycoproteomics/n_summary_plot.pdf", n_summary_plot, width = 3, height = 2)
tgutil::ggpreview(plot = n_summary_plot, width = 3, height = 2)
...
tribble(
  ...
  write_csv("results/source_data/Supplementary_Figure_16a.csv")
```
...
data <- read_pglyco3_pglycoquant(
  ...
  as_tibble() %>% 
  mutate(
    glycan_composition = as.character(glycan_composition),
    glycan_composition = convert(glycan_composition),
    ...
    first_protein = gene,
    first_protein_site = protein_site,
...
pooled_glycome_data <- read_csv("results/data/glyhunter_results_pooled/summary_area.csv") %>% 
...
pseudo_glycome_plot_data %>%
  filter(group == "HC") %>%
  write_csv("results/source_data/Figure_5a.csv")

pseudo_glycome_plot_data %>%
  write_csv("results/source_data/Supplementary_Figure_17.csv")
```

```{r}
H5N4S2_donut_data <- mean_data %>% 
...
hole_size <- 1.5
H5N4S2_donut <- ggplot(H5N4S2_donut_data, aes(x = hole_size, y = abundance, fill = glycosite)) +
  geom_col() +
  geom_text(
...
write_csv(H5N4S2_donut_data, "results/source_data/Figure_5b.csv")
```

```{r}
H5N5F1S1_donut_data <- mean_data %>% 
...
hole_size <- 1.5
H5N5F1S1_donut <- ggplot(H5N5F1S1_donut_data, aes(x = hole_size, y = abundance, fill = glycosite)) +
  geom_col() +
  geom_text(
    aes(label = scales::percent(rela_abundance, 0.1)),
    position = position_stack(vjust = 0.5),
...
write_csv(H5N5F1S1_donut_data, "results/source_data/Figure_5c.csv")
```
...
glycan_composition_plot_data <- mean_data %>% 
  filter(group == "HC") %>% 
...
glycan_composition_p <- glycan_composition_plot_data %>%
...
write_csv(glycan_composition_plot_data, "results/source_data/Figure_5d.csv")
```

```{r}
prepare_bar_for_selected <- function(glycan, n_sites) {
...
plot_bar_for_selected <- function(glycan, n_sites) {
  plot_data <- prepare_bar_for_selected(glycan, n_sites)
  ggplot() +
...
```{r}
prepare_bar_for_selected("H5N3S1", 5) %>%
  write_csv("results/source_data/Figure_5i.csv")

prepare_bar_for_selected("H7N6F1S2", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_18a.csv")

prepare_bar_for_selected("H7N6F1S4", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_18b.csv")

prepare_bar_for_selected("H7N6F1S3", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_18c.csv")

prepare_bar_for_selected("H6N5F1S3", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24a.csv")

prepare_bar_for_selected("H5N4F1", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24b.csv")

prepare_bar_for_selected("H3N4F1", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24c.csv")

prepare_bar_for_selected("H5N2", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24d.csv")

prepare_bar_for_selected("H6N5S3", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24e.csv")

prepare_bar_for_selected("H5N4S1", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_24f.csv")
```
...
double_volcano_plot_data %>%
  filter(contrast == "CHB-HC") %>%
  write_csv("results/source_data/Figure_5e.csv")
double_volcano_plot_data %>%
  filter(contrast == "LC-HC") %>%
  write_csv("results/source_data/Figure_5f.csv")
double_volcano_plot_data %>%
  filter(contrast == "HCC-HC") %>%
  write_csv("results/source_data/Figure_5g.csv")
```

```{r}
up_ssg_venn_data <- double_volcano_plot_data %>% 
  filter(regulate == "up") %>% 
  select(ssg, contrast) %>% 
  mutate(up = TRUE) %>% 
  pivot_wider(id_cols = ssg, names_from = contrast, values_from = up, values_fill = FALSE)
up_ssg_venn <- up_ssg_venn_data %>% 
  ggvenn::ggvenn(
...
down_ssg_venn_data <- double_volcano_plot_data %>% 
  filter(regulate == "down") %>% 
  select(ssg, contrast) %>% 
  mutate(up = TRUE) %>% 
  pivot_wider(id_cols = ssg, names_from = contrast, values_from = up, values_fill = FALSE)
down_ssg_venn <- down_ssg_venn_data %>% 
  ggvenn::ggvenn(
    show_percentage = FALSE, 
    fill_color = c("#A2AFA6", "#FEC37D", "#CC5F5A"),
    text_size = 6
...
down_gp_venn_data <- double_volcano_plot_data %>% 
  filter(regulate == "down") %>% 
  distinct(contrast, protein) %>% 
  mutate(up = TRUE) %>% 
  pivot_wider(id_cols = protein, names_from = contrast, values_from = up, values_fill = FALSE)

down_gp_venn <- down_gp_venn_data %>% 
  ggvenn::ggvenn(
    show_percentage = FALSE, 
    fill_color = c("#A2AFA6", "#FEC37D", "#CC5F5A"),
    text_size = 6
...
write_csv(up_ssg_venn_data, "results/source_data/Figure_5h_1.csv")
write_csv(down_ssg_venn_data, "results/source_data/Figure_5h_2.csv")

write_csv(up_ssg_venn_data, "results/source_data/Supplementary_Figure_19a.csv")
write_csv(down_ssg_venn_data, "results/source_data/Supplementary_Figure_19b.csv")
write_csv(up_gp_venn_data, "results/source_data/Supplementary_Figure_19c.csv")
write_csv(down_gp_venn_data, "results/source_data/Supplementary_Figure_19d.csv")
```
...
extract_dys_proteins <- function(group, name_type) {
  double_volcano_plot_data %>% 
    ...
    distinct(protein) %>% 
    left_join(protein_map, by = c("protein" = "SYMBOL")) %>% 
...
```{r}
go_bp_res <- read_csv("results/data/glycoproteomics/go_bp_result.csv")
write_csv(go_bp_res, "results/source_data/Supplementary_Figure_19e.csv")

kegg_res <- read_csv("results/data/glycoproteomics/kegg_result.csv")
write_csv(kegg_res, "results/source_data/Supplementary_Figure_19f.csv")
```

```{r}
prepare_site_pro_dual_ggs <- function(glycosite, n = 5) {
...
prepare_site_pro_dual_pro <- function(glycosite, n = 5) {
  protein <- str_extract(glycosite, "(.*) \\(.*\\)", group = 1)
  pro_data <- pro_data %>% 
...
prepare_site_pro_dual_ggs("A2MG (N869)", 5) %>%
  write_csv("results/source_data/Figure_5j_1.csv")
prepare_site_pro_dual_pro("A2MG (N869)", 5) %>%
  write_csv("results/source_data/Figure_5j_2.csv")
prepare_site_pro_dual_ggs("A1AG1 (N93)", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_18d_1.csv")
prepare_site_pro_dual_pro("A1AG1 (N93)", 5) %>%
  write_csv("results/source_data/Supplementary_Figure_18d_2.csv")
```
...
pro_corrected_struc_data <- struc_data |> 
  inner_join(
    pro_data |> select(protein, sample, pro_abund = abundance), 
    by = c("first_protein" = "protein", "sample")
    ) |> 
  filter(pro_abund > 0) |> 
  mutate(value = value / pro_abund, .keep = "unused")

struc_df <- pro_corrected_struc_data %>% 
...
prepared_for_decouple <- pro_corrected_struc_data %>% 
...
fut8_nglybai_mat <- nglybai_res %>% 
  ...
  pivot_wider(names_from = glycosite, values_from = score) %>%
  column_to_rownames("group") %>%
  as.matrix()
fut8_nglybai_mat <- fut8_nglybai_mat[rev(c("HC", "CHB", "LC", "HCC")),]

color_fun <- colorRamp2(c(-2, 2), c("#EBDAE9", "#64529B"))
ht <- Heatmap(
  fut8_nglybai_mat,
  name = "FUT8\nActivity\nScore",
  col = color_fun,
  ...
  row_split = 1:4,
  row_title = NULL,
  column_title = "Glycosite",
  width = unit(15, "cm"),
  height = unit(3, "cm"),
)

# pdf("results/figures/glycoproteomics/fut8_nglybai_heatmap.pdf", width = 15, height = 3)
# draw(ht)
# dev.off()
```
